...
#condor_status -pool vocms0207.cern.ch -any -const 'MyType=?="glidefactory" && GLIDEIN_Supported_VOs=?="CMSGPU" && regexp("_US_",GLIDEIN_CMSSite)' -af GLIDEIN_Gatekeeper | awk '{print $2}' > site_gatekeepers.txt
